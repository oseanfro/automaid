...
# @Last modified time: 2023-07-07T12:12:52+02:00
            ...
            if cycle.sbe61Profiles :
                for profile in cycle.sbe61Profiles :
                    if cycle.cycleNb < 1000 :
                        ...
                        profCFilePath = mfloat_nc_path + "R" +FloatWmoID + "_"+f"{cycle.cycleNb:04}"+".nc"
                    print(profCFilePath)
                    #information to retrieve from a possible existing multi-profile file
                    if os.path.exists(profCFilePath):
                        os.remove(profCFilePath)

                    file_cdf = Dataset(profCFilePath, "w", format="NETCDF3_CLASSIC")
                    ##################################################################################################
                    ###                                                                                             ##
                    ###                                     Create Dimensions                                       ##
                    ###                                                                                             ##
